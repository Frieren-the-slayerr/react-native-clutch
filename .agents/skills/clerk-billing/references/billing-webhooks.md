...
- On subscriptionItem events, the event data IS the item. There is no `subscription_id` back-reference — use `evt.data.id` (the item's own stable id) as the key to match or persist the record. Do not key off `payer` + `plan.slug`: a payer can have multiple items, and a plan slug can change, so that lookup can silently hit the wrong row.
...
export async function POST(req: NextRequest) {
	...
	if (evt.type === 'subscription.updated') {
		// Upsert, not update: if the create/active event was missed or this update
		// is replayed out of order, a plain update() throws on the missing row and
		// turns a webhook retry into a 500.
		const { id, payer, items, status } = evt.data
		const entityId = payer.organization_id ?? payer.user_id
		const plan = items[0]?.plan?.slug
		await db.subscriptions.upsert({
	...
	if (evt.type === 'subscriptionItem.canceled') {
		...
		// Key off the item's own id — it's stable even if the payer has multiple items
		// or the plan slug changes.
		const { id, canceled_at } = evt.data
