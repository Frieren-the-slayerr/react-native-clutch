...
ORG=$(bash scripts/execute-request.sh POST /organizations "{\"name\": \"Acme Corp\", \"created_by\": \"$USER_ID\"}")
...
bash scripts/execute-request.sh POST "/organizations/${ORG_ID}/invitations" "{\"email_address\": \"user@example.com\", \"role\": \"org:admin\"}" \
  | python3 -c "import sys,json; print(json.dumps(json.load(sys.stdin), indent=2))"
```

(add `--admin` as the first argument to `execute-request.sh` only after the user has explicitly confirmed bypassing scope checks — see CRITICAL section)
...
```bash
bash scripts/execute-request.sh PATCH "/users/${USER_ID}" '{"public_metadata": {"plan": "pro", "onboarded": true}}' \
...
```bash
bash scripts/execute-request.sh GET "/users?limit=100&offset=0&order_by=-created_at&created_at=gt:$(date -d '7 days ago' +%s 2>/dev/null || date -v-7d +%s)000" \
  | python3 -c "
import sys, json
...
bash scripts/execute-request.sh DELETE "/users/${USER_ID}" \
...
**ALWAYS execute requests through `scripts/execute-request.sh`.** It enforces `CLERK_BAPI_SCOPES` for write/delete requests (blocking POST/PUT/PATCH without a `write` scope and DELETE without `write,delete`, unless `--admin` is passed) and sources `CLERK_SECRET_KEY` from `.env`/`.env.local` automatically. Do NOT use raw `curl` for API execution — that bypasses scope enforcement entirely and turns the MANDATORY checks in the CRITICAL section into prose the agent could skip. (Raw `curl` is still fine for spec discovery — see `api-specs-context.sh`, `extract-tags.js`, `extract-endpoint-detail.sh` below — since those hit GitHub, not the Clerk API.)

Usage: `bash scripts/execute-request.sh [--admin] <METHOD> <PATH> [BODY]`

Template for GET requests:
```bash
bash scripts/execute-request.sh GET "${PATH}${QUERY_STRING}"
```

Template for POST/PATCH/PUT requests:
```bash
bash scripts/execute-request.sh ${METHOD} "${PATH}" '${BODY_JSON}'
```

Template for DELETE requests:
```bash
bash scripts/execute-request.sh DELETE "${PATH}"
```
...
- Execute all API calls through `scripts/execute-request.sh` — never use raw `curl` for API execution. The script is what enforces the `CLERK_BAPI_SCOPES` check above; bypassing it defeats that enforcement.
...
4. Execute the request via `scripts/execute-request.sh` (see How to execute requests above).
...
```bash
RESPONSE=$(bash scripts/execute-request.sh GET "/users?limit=10")
